...
class DelegationHandleTest(unittest.TestCase):
    def setUp(self):
        # ignore_cleanup_errors is a backstop for the deadline Timer, which is
        # not named and so cannot be joined below; it only ever leaves a temp
        # dir behind, never a failed test.
    ...
    def tearDown(self):
        self.gate.set()
        # The delegation worker is a daemon thread that keeps writing to
        # index.db after the test body returns. Windows refuses to unlink a
        # file another thread still has open, so cleanup() raced the worker and
        # this class failed intermittently in full-suite runs - on whichever
        # test happened to lose. Wait the workers out first.
        for thread in threading.enumerate():
            if thread.name.startswith("agent-delegate-"):
