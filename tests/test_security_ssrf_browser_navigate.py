...
class TestBrowserNavigateSSRF(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # execute() preflights that a browser engine is installed before it
        # dispatches to the navigate handler where the SSRF guard lives. That
        # preflight came later than these tests and probes the real host, so
        # without stubbing it every case here fails with the onboarding
        # message on any machine that has no engine installed - which is not
