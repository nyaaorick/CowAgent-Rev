"""Import the genuine `wcferry` package out of the WeChatFerry submodule.

Deliberately not `pip install wcferry`: the point of the harness is to exercise
the exact client and protobuf definitions this repo pins as a submodule, so a
version drift between the submodule and PyPI shows up as a test failure rather
than as a surprise on the Windows host.
"""
...
SUBMODULE_HINT = (
    f"wcferry client not found at {CLIENT_PATH}. The WeChatFerry submodule is "
...
try:
    ...
    IMPORT_ERROR = None
except Exception as e:  # pragma: no cover - only when the submodule is missing
    wcf_pb2 = None
    Wcf = None
    WxMsg = None
    IMPORT_ERROR = e
