...
from tests.support_fs import symlinks_supported


def _assert_shared_link(kdir: Path):
    """A shared knowledge dir is a symlink to the shared base.

    AgentAdminService._link_shared_knowledge documents a fallback for
    platforms that refuse symlinks (Windows without Developer Mode): it
    leaves nothing in place and resolution falls back to the shared copy.
    Pin whichever of the two the host can actually produce.
    """
