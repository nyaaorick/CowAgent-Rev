...
# Both localizations of AgentStreamExecutor._empty_response_fallback().
_FALLBACKS = (
    u"抱歉，我暂时无法生成回复。"
    u"请尝试换一种方式描述你的需求，或稍后再试。",
    "Sorry, I can't generate a reply right now. "
...
class EmptyResponseFallbackTest(unittest.TestCase):
    ...
    def test_empty_reply_gets_fallback_text_for_user_facing_run(self):
        ...
        # The fallback goes through _t(), so which of the two strings comes
        # back depends on the runtime language. Asserting only the Chinese one
        # made this fail on any host resolving cow_lang to en.
        self.assertIn(response, _FALLBACKS)
