...
class TestReadCredentialBypass(unittest.TestCase):
    ...
    def test_proc_self_environ_blocked(self):
        ...
        if os.name == "nt":
            # There is no /proc on Windows, so nothing to leak: read() first
            # resolves the path to a drive-relative "C:\proc\self\environ",
            # which is an ordinary (non-existent) file. The guard assertion
            # above is the part that carries over.
            self.skipTest("/proc is POSIX-only")
