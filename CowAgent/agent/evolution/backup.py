...
def create_backup(workspace_dir: Path, files: List[Path]) -> Optional[str]:
    ...
    try:
        ...
        for idx, src in enumerate(existing):
            ...
            try:
                # as_posix(): the manifest is a persisted format, and
                # restore does `ws / entry["rel"]`. A Windows-written
                # "skills\a.md" resolves fine on Windows but becomes one
                # filename containing a literal backslash on POSIX.
