...
def canonical_memory_path(path) -> str:
    """Canonical form of a workspace-relative path key: always POSIX.

    ``chunks.path`` / ``files.path`` are keys, matched with ``WHERE path = ?``,
    and their producers and consumers live in different modules. The memory
    manager derives them from ``Path.relative_to`` (OS-native separators) while
    KnowledgeService builds them by hand with "/", so on Windows a knowledge
    document deleted or moved never had its chunks removed - the delete matched
    nothing and the stale text kept coming back from memory search. Canonicalize
    on the way in so both sides agree regardless of platform.
    """
...
class MemoryChunk:
    ...
    def __post_init__(self):
        self.path = canonical_memory_path(self.path)
...
class MemoryStorage:
    ...
    def _quarantine_and_recreate(self):
        ...
        # Did the primary file actually get out of the way? Only then does the
        # reopen below produce an empty database, and only then is the old one
        # somewhere the operator can find it.
        moved_aside = False
        copied_aside = False
        for path in (
            self.db_path,
            ...
            if not path.exists():
                continue
            is_primary = path == self.db_path
            try:
                os.replace(str(path), f"{path}{suffix}")
                if is_primary:
                    moved_aside = True
            except OSError as e:
                # Windows refuses to rename a file any handle still has open,
                # and ConversationStore holds this very file open for the whole
                # process lifetime. Renaming is not available, but reading is,
                # so keep a byte copy: preserving the data is the entire point
                # of quarantining, and losing it is the one outcome the caller
                # documents as unacceptable.
                try:
                    shutil.copy2(str(path), f"{path}{suffix}")
                    if is_primary:
                        copied_aside = True
                except OSError as copy_error:
                    logger.error(
                        f"[MemoryStorage] Failed to quarantine {path}: {e} "
                        f"(copying it aside also failed: {copy_error})"
                    )

        if moved_aside:
            logger.error(
                f"[MemoryStorage] Corrupt database moved to {self.db_path}{suffix} "
                f"and replaced by an empty one. Conversation history can be "
                f"recovered from the quarantined copy."
            )
        elif copied_aside:
            logger.error(
                f"[MemoryStorage] Corrupt database could not be moved aside "
                f"(it is still open elsewhere in this process); a recoverable "
                f"copy was left at {self.db_path}{suffix}. The original is "
                f"still in place and still corrupt - restart the process to "
                f"let it be replaced."
            )
        else:
            logger.error(
                f"[MemoryStorage] Corrupt database at {self.db_path} could "
                f"NOT be quarantined and is still in place. Do not delete it: "
                f"it holds the only copy of the conversation history."
            )
    ...
    def _init_db(self):
        ...
        self._migrate_native_separator_paths()

        self.conn.commit()

    def _migrate_native_separator_paths(self):
        """Rewrite path keys written with OS-native separators to POSIX form.

        Paths are workspace-relative keys, and until they were canonicalized
        (see canonical_memory_path) a Windows run stored "knowledge\\a.md"
        while KnowledgeService deleted by "knowledge/a.md" - so removing or
        moving a document left its chunks in the index for good.

        The path is rewritten rather than the rows dropped: chunks are
        re-derivable, but their embeddings are not free to recompute. Chunk ids
        derive from the path and are deliberately left alone; the next sync
        finds the file hash unchanged under the canonical key and skips it, and
        a sync that does find a change deletes every row for that path before
        reinserting, so no duplicate can survive either way.
        """
        try:
            for table in ("chunks", "files"):
                self.conn.execute(
                    f"UPDATE {table} SET path = REPLACE(path, '\\', '/') "
                    f"WHERE path LIKE '%\\%'"
                )
        except sqlite3.DatabaseError as e:
            from common.log import logger
            logger.warning(f"[MemoryStorage] Path separator migration skipped: {e}")
