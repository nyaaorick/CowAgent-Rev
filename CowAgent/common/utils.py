...
def expand_path(path: str) -> str:
    ...
    # expanduser('~/cow') splices the home directory onto the literal
    # remainder, so on Windows it returns 'C:\\Users\\me/cow' - a
    # mixed-separator path that leaks into user-facing log messages and fails
    # string comparisons against a path built with os.path.join. Normalize
    # only what we actually expanded, so a relative path handed in unchanged
    # stays byte-identical for callers that compare on it.
    if path.startswith('~'):
