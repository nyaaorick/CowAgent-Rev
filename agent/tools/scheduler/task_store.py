        ...
        with self.lock:
            try:
                ...
                if os.path.exists(self.store_path):
                    ...
                    try:
                        # A binary copy, not a text one. The store is written
                        # as UTF-8 with ensure_ascii=False, so a task described
                        # in Chinese put non-ASCII bytes on disk that the
                        # locale codec could not decode. Opening the
                        # destination in 'w' mode had already truncated the
                        # previous good backup by the time the read raised, and
                        # the bare `except: pass` hid it - every save left a
                        # 0-byte .bak and said nothing.
                        shutil.copy2(self.store_path, backup_path)
                    except Exception as e:
                        print(f"Warning: could not back up {self.store_path}: {e}")
