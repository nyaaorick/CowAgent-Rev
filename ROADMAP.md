...
| **WCF-BUG-02** | `spy.dll` (`offsets.h`) | `AccountStorageMgr` `sqlite3*` handle array offset was `storage + 0x34` in older WeChat, but moved to `storage + 0x38` in WeChat 3.9.12.56; unpatched `spy.dll` reads NULL (`0x00`), so `MicroMsg.db` is never opened. | `get_contacts()` returns 0 contacts; contact nicknames fall back to raw `wxid`. | **[RESOLVED]** Patched byte at file offset `0x20dd4` from `0x34` to `0x38` (`8D 42 34` -> `8D 42 38`) in `.venv/Lib/site-packages/wcferry/spy.dll`. `MicroMsg.db` now opens natively, restoring all 6,000+ contacts with WXID, Alias, and Remarks. See [`docs/WCF_WECHAT_3.9.12.56_REPAIR.md`](docs/WCF_WECHAT_3.9.12.56_REPAIR.md). |
...
- [x] **5.2 Contact & Database Diagnostics**
  - Resolved `AccountStorageMgr` offset for WeChat 3.9.12.56 (`storage + 0x38`), permanently restoring `MicroMsg.db` handle and `get_contacts()` nickname resolution. See [`docs/WCF_WECHAT_3.9.12.56_REPAIR.md`](docs/WCF_WECHAT_3.9.12.56_REPAIR.md).
